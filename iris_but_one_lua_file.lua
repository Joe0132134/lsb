function temp_require(lsb)
	return temp_requires[lsb]()
end
...
temp_requires = {
	...
	API = function()
		local Types = temp_require("Types")
	...
	Internal = function()
		...
		return function(Iris: Types.Iris): Types.Internal
			...
			function Internal.WidgetConstructor(type: string, widgetClass: Types.WidgetClass)
				...
				for _, field: string in Fields.All.temp_required do
					assert(widgetClass[field] ~= nil, `field {field} is missing from widget {type}, it is temp_required for all widgets`)
				...
				if widgetClass.hasState then
					for _, field: string in Fields.IfState.temp_required do
						assert(widgetClass[field] ~= nil, `field {field} is missing from widget {type}, it is temp_required for all widgets with state`)
						thisWidget[field] = widgetClass[field]
					end
				...
				if widgetClass.hasChildren then
					for _, field: string in Fields.IfChildren.temp_required do
						assert(widgetClass[field] ~= nil, `field {field} is missing from widget {type}, it is temp_required for all widgets with children`)
						thisWidget[field] = widgetClass[field]
					end
	...
	widgets = function()
		...
		return function(Iris: Types.Internal)
			...
			temp_require("m_Root")(Iris, widgets)
			temp_require("m_Window")(Iris, widgets)

			temp_require("m_Menu")(Iris, widgets)

			temp_require("m_Format")(Iris, widgets)

			temp_require("m_Text")(Iris, widgets)
			temp_require("m_Button")(Iris, widgets)
			temp_require("m_Checkbox")(Iris, widgets)
			temp_require("m_RadioButton")(Iris, widgets)

			temp_require("m_Tree")(Iris, widgets)

			temp_require("m_Input")(Iris, widgets)
			temp_require("m_Combo")(Iris, widgets)
			temp_require("m_Plot")(Iris, widgets)

			temp_require("m_Table")(Iris, widgets)
		end
	end,
	m_Button = function()
		local Types = temp_require("Types")
				...
				GenerateState = function(thisWidget: Types.Widget)
					if thisWidget.state.index == nil then
						if thisWidget.arguments.Index ~= nil then
							error("a shared state index is temp_required for Selectables with an Index argument", 5)
						end
				...
				Update = function(thisWidget: Types.Widget)
					...
					if thisWidget.InitialNumColumns == -1 then
						if thisWidget.arguments.NumColumns == nil then
							error("Iris.Table NumColumns argument is temp_required", 5)
						end
			...
			Iris.WidgetConstructor("Text", {
				...
				Update = function(thisWidget: Types.Widget)
					...
					if thisWidget.arguments.Text == nil then
						error("Iris.Text Text Argument is temp_required", 5)
					end
...
function Iris()
	...
	local Internal: Types.Internal = temp_require("Internal")(Iris)
	...
	Iris.TemplateConfig = temp_require("config")
	...
	Iris.ShowDemoWindow = temp_require("demoWindow")(Iris)

	temp_require("widgets")(Internal)
	temp_require("API")(Iris)
